...
async function placeOrder(userId, cartObj) {
    ...
    let userPromise = getUserDetails(userId)
    let productPromise = getProductDetails(products)

    let userResult = await userPromise
    let productDetails = await productPromise

    let pincode = userResult.addresses[cartObj.addressId].pincode

    if (!pincode) {
        throw new Error("Invalid address found")
    }

    if (!await isDeliverable(pincode, products)) {
        throw new Error("Products not deliverable to this address")
    }

    let promoResult = await applyPromocode(userId, cartObj, productDetails)
    if (!promoResult.applied) {
        throw new Error("Promocode not applicable")
    }

    let orderResult = await createOrder(userId, cartObj, promoResult)
    consumeInventory(userId, cartObj)
    return orderResult
}
